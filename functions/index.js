...
const verificationWebhookSecret = defineSecret('ADULT_VERIFICATION_WEBHOOK_SECRET');
const verificationStateSecret = defineSecret('PARENT_VERIFICATION_STATE_SECRET');
const appBaseUrl = defineString('APP_BASE_URL');
const consentFromEmail = defineString('PARENT_CONSENT_FROM_EMAIL');
const adultVerificationStartUrl = defineString('ADULT_VERIFICATION_START_URL');
...
export const finalizeAdultConsent = onCall(callableOptions, async (request) => {
...
export const createParentConsentRequest = onCall(
...
export const getParentConsentRequest = onCall(callableOptions, async (request) => {
...
export const authenticateParentForConsent = onCall(callableOptions, async (request) => {
  const actor = authenticatedGoogleUser(request);
  const invitation = await resolveInvitation(request.data?.token);
...
export const startParentAdultVerification = onCall(
...
export const parentAdultVerificationWebhook = onRequest(
...
export const captureParentConsent = onCall(
  { ...callableOptions, secrets: [resendApiKey] },
  async (request) => {
    const actor = authenticatedGoogleUser(request);
    const data = request.data || {};
...
export const claimChildConsent = onCall(callableOptions, async (request) => {
  const actor = authenticatedGoogleUser(request);
...
export const submitDataPrincipalRequest = onCall(
  { ...callableOptions, secrets: [resendApiKey] },
  async (request) => {
...
export const listDataPrincipalRequests = onCall(callableOptions, async (request) => {
  const actor = request.auth;
  if (!actor || !['ADULT_CONSENTED', 'PARENT_VERIFIED'].includes(actor.token.privacyStatus)) {
    throw new HttpsError('permission-denied', 'A consent-authorized account is required.');
  }
...
export const getChildRightsApproval = onCall(callableOptions, async (request) => {
...
export const approveChildRightsRequest = onCall(callableOptions, async (request) => {
  const actor = authenticatedGoogleUser(request);
  const item = await resolveRightsApproval(request.data?.token);
...
export const processVerifiedRightsRequests = onSchedule(
  {
    region: REGION,
...
export const deleteExpiredPrivacyArtifacts = onSchedule(
